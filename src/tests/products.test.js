...
describe('Products', () => {
  describe('GET /products', () => {
    ...
    test('Should get related products by id', async () => {
      ...
      assert.ok(body.every(product => product.category.id === body[0].category.id && product.id !== id), "All products should be of the same category and shouldn't include the searched id product")
    })
    ...
    test('Should get related products by slug', async () => {
      ...
      assert.strictEqual(status, 200, 'Should return a 200 status')
      assert.ok(Array.isArray(body), 'Should return an array')
      assert.ok(body.every(product => product.category.id === body[0].category.id && product.slug !== slug), 'All products should be of the same category')
    })
    ...
    describe('Filter products', () => {
      ...
      test('Should filter by price range', async () => {
        const minPrice = 10
        const maxPrice = 20
        const { status, body } = await request(app).get(`/products?minPrice=${minPrice}&maxPrice=${maxPrice}`).send()
        assert.strictEqual(status, 200, 'Should return a 200 status')
        assert.ok(
  ...
  describe('POST /products', () => {
    ...
    test('Should fail the creation and return a type error from the title', async () => {
      ...
      assert.ok(body.error.errors.some(val => val.path.includes('title')), 'The errors should include a title error')
    })
  })
  ...
  describe('PATCH /products', () => {
    test('Should modified partially a product', async () => {
      ...
      const response = await request(app).patch('/products/6f76662d-a667-431b-a441-187c6cb37c21').send({ originalPrice })
    ...
    test('Should return an error patching a product', async () => {
      const response = await request(app).patch('/products/6f76662d-a667-431b-a441-187c6cb37c21').send({ originalPrice: '140' })
      assert.strictEqual(response.status, 400, 'Should return a 400 status')
