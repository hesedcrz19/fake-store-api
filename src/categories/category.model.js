...
export class CategoryModel {
  ...
  static async getProductsById ({ id, filters }) {
    const category = categories.find(category => category.id === id)
    if (!category) {
      throw createError({
        code: ERROR_CODES.CATEGORY_NOT_FOUND,
  ...
  static async getProductsBySlug ({ slug, filters }) {
    const category = categories.find(category => category.slug === slug)
    if (!category) {
      throw createError({
        code: ERROR_CODES.CATEGORY_NOT_FOUND,
  ...
  static async update ({ id, body }) {
    ...
    const prevCategoryIndex = categories.findIndex(
    ...
    if (
      categories.find((category, i) => category.slug === newCategory.slug && i !== prevCategoryIndex)
    ) {
      throw createError({
        code: ERROR_CODES.TITLE_ALREADY_EXIST,
        message: 'The category name already exist',
