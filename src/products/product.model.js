...
export class ProductModel {
  ...
  static async getRelatedById ({ id, filters }) {
    ...
    const relatedProducts = products.filter(el => el.category.id === product.category.id && el.id !== product.id)
  ...
  static async update ({ id, body }) {
    ...
    const prevProductIndex = products.findIndex(
    ...
    if (
      products.find((product, i) => product.slug === newProduct.slug && i !== prevProductIndex)
    ) {
      throw createError({
        code: ERROR_CODES.TITLE_ALREADY_EXIST,
        message: 'The product title already exist',
