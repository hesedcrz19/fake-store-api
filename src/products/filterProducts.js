...
export const filterProducts = (
  ...
  if (title) {
    products = products.filter(
      product => product.title.toLowerCase().includes(title)
    )
  }
  ...
  if (discountPercentage) {
    products = products.filter(
      product => product.discountPercentage === discountPercentage
    )
  }

  if (minDiscountPercentage) {
    products = products.filter(
      product => product.discountPercentage >= minDiscountPercentage
    )
  }
  ...
  if (hasDiscount !== undefined) {
    products = products.filter(
      product => hasDiscount ? product.discountPercentage > 0 : product.discountPercentage === 0
    )
  }

  if (freeShipping !== undefined) {
    products = products.filter(
      product => freeShipping ? product.shippingCost === 0 : product.shippingCost > 0
    )
  }

  if (hasPromotion !== undefined) {
    products = products.filter(
      product => hasPromotion ? product.promotion : !product.promotion
    )
  }
